...
typeField.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space") {
    ...
    if (typeField.value != "") {
      ...
      console.log(
        `onTypedSubArrayIndex: ${onTypedSubArrayIndex} \ttypedArray length: ${typedArray.length} \ntypedSubArray: ${typedSubArray}`
      );
  ...
  if (typeField.value === "" && e.code === "Backspace") {
    ...
    if (onTypedSubArrayIndex === 0 && typedArray.length === 0) {
      ...
      wordsArray[wordsArrayIndex][onTypedSubArrayIndex] = extract(
        wordsArray[wordsArrayIndex][onTypedSubArrayIndex]
      );
      //removing the space at the end
      if (
        wordsArray[wordsArrayIndex][onTypedSubArrayIndex] ==
        typedSubArray[onTypedSubArrayIndex].slice(0, -1)
      ) {
  ...
  wpm_calc = parseFloat(
    (letters - letter_penalty) / 5 / ((calc_seconds - seconds) / 60)
  );
...
function replaceWord(wordArray, typed) {
  if (wordArray[onTypedSubArrayIndex] === typed) {
    ...
    wordArray.splice(
      onTypedSubArrayIndex,
      1,
      `<span style = "color: rgb(71, 255, 65)">${wordArray[onTypedSubArrayIndex]}</span>`
    );
  } else {
    wrongWords++;
    ...
    wordArray.splice(
      onTypedSubArrayIndex,
      1,
      `<span style = "color: red">${wordArray[onTypedSubArrayIndex]}</span>`
    );
  }
}
...
function showVar() {
  console.log(
    `wordsArrayIndex: ${wordsArrayIndex} \t onTypedSubArrayIndex: ${onTypedSubArrayIndex} \t letters: ${letters} \t wrongWords: ${wrongWords} \n onArray: ${onArray}`
  );
}
